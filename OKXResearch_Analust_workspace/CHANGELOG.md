...
#### ✨ New Features
*   **📰 Intelligence Center**: Added `src/api/news_client.py` integrating CryptoPanic API.
    *   **Smart Filtering**: Automatically prioritizes "Important" news, with a fallback to "Hot" if no major events are found.
    *   **AI Verification**: DeepSeek-R1 now cross-references news with market data to identify "news-price divergence" and filters out noise.
    *   **Structured Output**: News is presented in a clean Markdown table with columns for Signal (🟢/🔴), Source, and AI-deduced Logic.
*   **🛡️ Robust Startup**:
    *   Updated `run.sh` to include a **Process Conflict Check** (prevents multiple instances).
    *   Added auto-cleaning of `__pycache__` to solve stubborn ImportError issues during updates.

#### 🔧 Refactoring
*   **Code Cleanup**: Removed experimental "Paper Trading" module to keep the codebase focused on analysis and reporting.
*   **Bug Fixes**: Fixed indentation errors in `main.py` and logic flaws in news table rendering (handling newlines and pipes).
